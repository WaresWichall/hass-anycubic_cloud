MQTT_HOST = "mqtt-universe.anycubic.com"
MQTT_PORT = 8883

MQTT_ROOT_TOPIC_PLUS = 'anycubic/anycubicCloud/v1/+/public/'
MQTT_ROOT_TOPIC_PRINTER = 'anycubic/anycubicCloud/v1/printer/app/'
MQTT_ROOT_TOPIC_PUBLISH = 'anycubic/anycubicCloud/v1/app/'
MQTT_ROOT_TOPIC_PUBLISH_PRINTER = 'anycubic/anycubicCloud/v1/printer/public/'
MQTT_ROOT_TOPIC_SERVER = 'anycubic/anycubicCloud/v1/server/app/'

MQTT_TIMEOUT = 60 * 20
MQTT_SUBSCRIBE_TIMEOUT = 15
MQTT_MESSAGE_TIMEOUT = 30
MQTT_RECONNECT_SECONDS_SHORT = 5
MQTT_RECONNECT_SECONDS_LONG = 20
